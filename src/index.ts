...
const liuyaoTrigramNames = ['乾', '兑', '离', '震', '巽', '坎', '艮', '坤'] as const;
type LiuyaoTrigram = typeof liuyaoTrigramNames[number];
type LiuyaoBinary = 0 | 1;
type LiuyaoLineNumber = 1 | 2 | 3 | 4 | 5 | 6;
type LiuyaoRelative = '父母' | '兄弟' | '子孙' | '妻财' | '官鬼';

interface LiuyaoStemBranch {
	stem: string;
	branch: string;
}

interface LiuyaoHexagram {
	name: string;
	upper: LiuyaoTrigram;
	lower: LiuyaoTrigram;
	lines: [LiuyaoBinary, LiuyaoBinary, LiuyaoBinary, LiuyaoBinary, LiuyaoBinary, LiuyaoBinary];
	palace: LiuyaoTrigram;
	palaceElement: ElementName;
	stage: string;
	worldLine: LiuyaoLineNumber;
	responseLine: LiuyaoLineNumber;
}

interface LiuyaoLineRecord {
	line: LiuyaoLineNumber;
	yaoText: string;
	sixSpirit: string;
...
const liuyaoLineNumbers = [1, 2, 3, 4, 5, 6] as const;
const liuyaoLineLabels: Record<LiuyaoLineNumber, string> = {
	1: '初爻',
	2: '二爻',
	3: '三爻',
	4: '四爻',
...
const liuyaoTrigramLines: Record<LiuyaoTrigram, [LiuyaoBinary, LiuyaoBinary, LiuyaoBinary]> = {
	乾: [1, 1, 1],
	兑: [1, 1, 0],
	离: [1, 0, 1],
	震: [1, 0, 0],
...
const liuyaoTrigramNajia: Record<LiuyaoTrigram, { inner: [LiuyaoStemBranch, LiuyaoStemBranch, LiuyaoStemBranch]; outer: [LiuyaoStemBranch, LiuyaoStemBranch, LiuyaoStemBranch] }> = {
	乾: {
...
] as const satisfies ReadonlyArray<{ name: string; upper: LiuyaoTrigram; lower: LiuyaoTrigram }>;

const liuyaoPalaceDefs = [
	{ palace: '乾', element: '金', hexagrams: [['乾为天', '本宫', 6], ['天风姤', '一世', 1], ['天山遁', '二世', 2], ['天地否', '三世', 3], ['风地观', '四世', 4], ['山地剥', '五世', 5], ['火地晋', '游魂', 4], ['火天大有', '归魂', 3]] },
	{ palace: '兑', element: '金', hexagrams: [['兑为泽', '本宫', 6], ['泽水困', '一世', 1], ['泽地萃', '二世', 2], ['泽山咸', '三世', 3], ['水山蹇', '四世', 4], ['地山谦', '五世', 5], ['雷山小过', '游魂', 4], ['雷泽归妹', '归魂', 3]] },
	{ palace: '离', element: '火', hexagrams: [['离为火', '本宫', 6], ['火山旅', '一世', 1], ['火风鼎', '二世', 2], ['火水未济', '三世', 3], ['山水蒙', '四世', 4], ['风水涣', '五世', 5], ['天水讼', '游魂', 4], ['天火同人', '归魂', 3]] },
	{ palace: '震', element: '木', hexagrams: [['震为雷', '本宫', 6], ['雷地豫', '一世', 1], ['雷水解', '二世', 2], ['雷风恒', '三世', 3], ['地风升', '四世', 4], ['水风井', '五世', 5], ['泽风大过', '游魂', 4], ['泽雷随', '归魂', 3]] },
	{ palace: '巽', element: '木', hexagrams: [['巽为风', '本宫', 6], ['风天小畜', '一世', 1], ['风火家人', '二世', 2], ['风雷益', '三世', 3], ['天雷无妄', '四世', 4], ['火雷噬嗑', '五世', 5], ['山雷颐', '游魂', 4], ['山风蛊', '归魂', 3]] },
	{ palace: '坎', element: '水', hexagrams: [['坎为水', '本宫', 6], ['水泽节', '一世', 1], ['水雷屯', '二世', 2], ['水火既济', '三世', 3], ['泽火革', '四世', 4], ['雷火丰', '五世', 5], ['地火明夷', '游魂', 4], ['地水师', '归魂', 3]] },
	{ palace: '艮', element: '土', hexagrams: [['艮为山', '本宫', 6], ['山火贲', '一世', 1], ['山天大畜', '二世', 2], ['山泽损', '三世', 3], ['火泽睽', '四世', 4], ['天泽履', '五世', 5], ['风泽中孚', '游魂', 4], ['风山渐', '归魂', 3]] },
	{ palace: '坤', element: '土', hexagrams: [['坤为地', '本宫', 6], ['地雷复', '一世', 1], ['地泽临', '二世', 2], ['地天泰', '三世', 3], ['雷天大壮', '四世', 4], ['泽天夬', '五世', 5], ['水天需', '游魂', 4], ['水地比', '归魂', 3]] },
] as const satisfies ReadonlyArray<{
	palace: LiuyaoTrigram;
	element: ElementName;
	hexagrams: ReadonlyArray<readonly [string, string, LiuyaoLineNumber]>;
}>;

const liuyaoPureHexagramNames: Record<LiuyaoTrigram, string> = {
	乾: '乾为天',
	兑: '兑为泽',
	离: '离为火',
	震: '震为雷',
...
const liuyaoGenerates: Record<ElementName, ElementName> = {
	木: '火',
	火: '土',
	土: '金',
	金: '水',
...
const liuyaoControls: Record<ElementName, ElementName> = {
	木: '土',
	土: '水',
	水: '火',
	火: '金',
...
const liuyaoSixSpiritCycle = ['青龙', '朱雀', '勾陈', '螣蛇', '白虎', '玄武'];

const liuyaoSixSpiritStartByDayStem: Record<string, number> = {
	甲: 0,
	乙: 0,
	丙: 1,
	丁: 1,
...
const liuyaoChangshengByElement: Record<ElementName, Record<string, string>> = {
...
function normalizeHexagramName(name: string) {
	return name
		.trim()
		.replace(/\s+/g, '')
		.replace(/[（）()【】\[\]]/g, '')
...
const liuyaoPalaceByHexagram = (() => {
	const map = new Map<string, { palace: LiuyaoTrigram; element: ElementName; stage: string; worldLine: LiuyaoLineNumber }>();
	for (const palaceDef of liuyaoPalaceDefs) {
		for (const [name, stage, worldLine] of palaceDef.hexagrams) {
			map.set(normalizeHexagramName(name), {
				palace: palaceDef.palace,
				element: palaceDef.element,
				stage,
				worldLine,
			});
		}
...
const liuyaoHexagramByName = (() => {
	const map = new Map<string, LiuyaoHexagram>();
	for (const def of liuyaoHexagramDefs) {
		const palace = liuyaoPalaceByHexagram.get(normalizeHexagramName(def.name));
		if (!palace) throw new Error(`Missing Liuyao palace definition: ${def.name}`);
		const lowerLines = liuyaoTrigramLines[def.lower];
		const upperLines = liuyaoTrigramLines[def.upper];
		const worldLine = palace.worldLine;
		const hexagram: LiuyaoHexagram = {
			name: def.name,
			upper: def.upper,
			lower: def.lower,
			lines: [lowerLines[0], lowerLines[1], lowerLines[2], upperLines[0], upperLines[1], upperLines[2]],
			palace: palace.palace,
			palaceElement: palace.element,
			stage: palace.stage,
			worldLine,
			responseLine: (((worldLine + 2) % 6) + 1) as LiuyaoLineNumber,
		};
		map.set(normalizeHexagramName(def.name), hexagram);
	}

	for (const trigram of liuyaoTrigramNames) {
		map.set(normalizeHexagramName(trigram), map.get(normalizeHexagramName(liuyaoPureHexagramNames[trigram]))!);
	}
	map.set(normalizeHexagramName('火雷噬磕'), map.get(normalizeHexagramName('火雷噬嗑'))!);
	return map;
})();

function parseLiuyaoHexagram(name: string, label: string) {
	const normalized = normalizeHexagramName(name);
	const hexagram = liuyaoHexagramByName.get(normalized);
	if (!hexagram) {
		throw new Error(`Unknown ${label}: ${name}. Use a standard 64-hexagram name, such as 乾为天、火地晋、雷泽归妹.`);
	}
	return hexagram;
}

function liuyaoNajiaLines(hexagram: LiuyaoHexagram) {
	const lower = liuyaoTrigramNajia[hexagram.lower].inner;
	const upper = liuyaoTrigramNajia[hexagram.upper].outer;
	return [lower[0], lower[1], lower[2], upper[0], upper[1], upper[2]] as [LiuyaoStemBranch, LiuyaoStemBranch, LiuyaoStemBranch, LiuyaoStemBranch, LiuyaoStemBranch, LiuyaoStemBranch];
}

function liuyaoBranchElement(branch: string): ElementName {
	return EarthBranch.fromName(branch).getElement().getName() as ElementName;
}

function liuyaoRelative(selfElement: ElementName, lineElement: ElementName): LiuyaoRelative {
	if (selfElement === lineElement) return '兄弟';
	if (liuyaoGenerates[lineElement] === selfElement) return '父母';
	if (liuyaoGenerates[selfElement] === lineElement) return '子孙';
	if (liuyaoControls[lineElement] === selfElement) return '官鬼';
	if (liuyaoControls[selfElement] === lineElement) return '妻财';
	throw new Error(`Unsupported element relation: ${selfElement}/${lineElement}`);
}

function liuyaoSixSpirit(dayStem: string, line: LiuyaoLineNumber) {
	const start = liuyaoSixSpiritStartByDayStem[dayStem];
	if (start === undefined) throw new Error(`Unsupported day stem for six spirits: ${dayStem}`);
	return liuyaoSixSpiritCycle[(start + line - 1) % liuyaoSixSpiritCycle.length];
}

function liuyaoYaoText(line: LiuyaoLineNumber, binary: LiuyaoBinary) {
	return `${liuyaoLineLabels[line]} ${binary === 1 ? '━━━ 阳' : '━ ━ 阴'}`;
}

function liuyaoStemBranchText(line: Pick<LiuyaoLineRecord, 'stem' | 'branch' | 'element'>) {
	return `${line.stem}${line.branch}（${line.element}）`;
}

function liuyaoRelativeStemBranchText(line: Pick<LiuyaoLineRecord, 'relative' | 'stem' | 'branch' | 'element'>) {
	return `${line.relative}${line.stem}${line.branch}${line.element}`;
}

function liuyaoKongwangText(cycle: SixtyCycle) {
	return cycle.getExtraEarthBranches().map((item) => item.getName()).join('');
}

function liuyaoChangsheng(element: ElementName, branch: string) {
	return liuyaoChangshengByElement[element][branch] ?? '-';
}

function formatLiuyaoChangsheng(element: ElementName, monthBranch: string, dayBranch: string) {
	return `月:${liuyaoChangsheng(element, monthBranch)} 日:${liuyaoChangsheng(element, dayBranch)}`;
}

function calculateLiuyaoShensha(
	line: Pick<LiuyaoLineRecord, 'stem' | 'branch'>,
	eightChar: EightChar,
) {
	const pillars = getPillarCycles(eightChar);
	const hits: string[] = [];
	const yearStem = pillars.year.getHeavenStem().getName();
	const dayStem = pillars.day.getHeavenStem().getName();
	const yearBranch = pillars.year.getEarthBranch().getName();
	const monthBranch = pillars.month.getEarthBranch().getName();
	const dayBranch = pillars.day.getEarthBranch().getName();
	const basisValues = {
		yearStem: { label: '年干', value: yearStem },
		dayStem: { label: '日干', value: dayStem },
		yearBranch: { label: '年支', value: yearBranch },
		dayBranch: { label: '日支', value: dayBranch },
	};

	if (line.branch === monthBranch) hits.push('临月');
	if (line.branch === dayBranch) hits.push('临日');
	if (EarthBranch.fromName(monthBranch).getOpposite().getName() === line.branch) hits.push('月破');
	if (EarthBranch.fromName(dayBranch).getOpposite().getName() === line.branch) hits.push('日冲');
	if (EarthBranch.fromName(monthBranch).getCombine().getName() === line.branch) hits.push('月合');
	if (EarthBranch.fromName(dayBranch).getCombine().getName() === line.branch) hits.push('日合');
	if (pillars.day.getExtraEarthBranches().some((branch) => branch.getName() === line.branch)) hits.push('日空');

	for (const rule of stemBranchShenshaRules) {
		for (const basis of rule.basis) {
			const basisValue = basisValues[basis];
			if ((rule.targets[basisValue.value] ?? []).includes(line.branch)) {
	...
	for (const rule of branchGroupShenshaRules) {
		for (const basis of rule.basis) {
			const basisValue = basisValues[basis];
			if (groupTarget(basisValue.value, rule.targetsByGroup) === line.branch) {
				hits.push(`${basisValue.label}${basisValue.value}:${rule.name}`);
			}
		}
	}
	...
	for (const rule of monthBranchShenshaRules) {
		const targets = rule.targets[monthBranch] ?? [];
		if (targets.some((target) => (target.type === 'stem' ? line.stem === target.name : line.branch === target.name))) {
...
function buildLiuyaoLineRecords(
	hexagram: LiuyaoHexagram,
	baseHexagram: LiuyaoHexagram,
	eightChar: EightChar,
	movingLines: Set<LiuyaoLineNumber>,
	includeMoving: boolean,
) {
	const najia = liuyaoNajiaLines(hexagram);
	const dayStem = eightChar.getDay().getHeavenStem().getName();
	const monthBranch = eightChar.getMonth().getEarthBranch().getName();
	const dayBranch = eightChar.getDay().getEarthBranch().getName();
	return liuyaoLineNumbers.map((line, index): LiuyaoLineRecord => {
		const stemBranch = najia[index];
		const element = liuyaoBranchElement(stemBranch.branch);
		const record: LiuyaoLineRecord = {
			line,
			yaoText: liuyaoYaoText(line, hexagram.lines[index]),
			sixSpirit: liuyaoSixSpirit(dayStem, line),
			relative: liuyaoRelative(baseHexagram.palaceElement, element),
			stem: stemBranch.stem,
			branch: stemBranch.branch,
			element,
			worldResponse: line === baseHexagram.worldLine ? '世' : line === baseHexagram.responseLine ? '应' : '',
			hidden: '-',
			isMoving: includeMoving && movingLines.has(line),
			shensha: '-',
			changsheng: formatLiuyaoChangsheng(element, monthBranch, dayBranch),
		};
		record.shensha = calculateLiuyaoShensha(record, eightChar);
		return record;
	});
}

function calculateLiuyaoHiddenGods(mainLines: LiuyaoLineRecord[], baseHexagram: LiuyaoHexagram, eightChar: EightChar) {
	const presentRelatives = new Set(mainLines.map((line) => line.relative));
	const pureHexagram = liuyaoHexagramByName.get(normalizeHexagramName(liuyaoPureHexagramNames[baseHexagram.palace]));
	if (!pureHexagram) throw new Error(`Missing pure palace hexagram for ${baseHexagram.palace}`);
	const pureLines = buildLiuyaoLineRecords(pureHexagram, baseHexagram, eightChar, new Set(), false);
	const hidden = new Map<LiuyaoLineNumber, string[]>();
	for (const pureLine of pureLines) {
		if (!presentRelatives.has(pureLine.relative)) {
			hidden.set(pureLine.line, [...(hidden.get(pureLine.line) ?? []), liuyaoRelativeStemBranchText(pureLine)]);
		}
	}
	return hidden;
}

function withLiuyaoHiddenGods(lines: LiuyaoLineRecord[], hidden: Map<LiuyaoLineNumber, string[]>) {
	return lines.map((line) => ({
		...line,
		hidden: hidden.get(line.line)?.join('、') ?? '-',
	}));
}

function findLiuyaoMovingLines(original: LiuyaoHexagram, changed: LiuyaoHexagram) {
	return liuyaoLineNumbers.filter((line, index) => original.lines[index] !== changed.lines[index]);
}

function formatLiuyaoBodyLine(line: LiuyaoLineRecord) {
	return `${liuyaoLineLabels[line.line]} ${line.relative}${line.stem}${line.branch}${line.element}`;
}

function formatLiuyaoBodies(baseHexagram: LiuyaoHexagram, mainLines: LiuyaoLineRecord[]) {
	const worldBinary = baseHexagram.lines[baseHexagram.worldLine - 1];
	const startIndex = worldBinary === 1 ? branchOrder.indexOf('子') : branchOrder.indexOf('午');
	const bodyBranch = branchOrder[(startIndex + baseHexagram.worldLine - 1) % branchOrder.length];
	const bodyHits = mainLines.filter((line) => line.branch === bodyBranch).map(formatLiuyaoBodyLine);
	const worldBranch = mainLines[baseHexagram.worldLine - 1].branch;
	const worldBodyLine = ((branchOrder.indexOf(worldBranch) % 6) + 1) as LiuyaoLineNumber;
	const worldBody = mainLines[worldBodyLine - 1];
	return {
		guaBody: `${bodyBranch}${bodyHits.length > 0 ? `（${bodyHits.join('、')}）` : '（不上卦）'}`,
		worldBody: `${liuyaoLineLabels[worldBodyLine]}（${formatLiuyaoBodyLine(worldBody)}）`,
	};
}
...
function buildLiuyaoChartData(question: string, datetime: string, originalHexagramName: string, changedHexagramName: string) {
	const { eightChar } = buildBaziContext(datetime);
	const originalHexagram = parseLiuyaoHexagram(originalHexagramName, 'originalHexagram');
	const changedHexagram = parseLiuyaoHexagram(changedHexagramName, 'changedHexagram');
	const movingLineList = findLiuyaoMovingLines(originalHexagram, changedHexagram);
	const movingLines = new Set(movingLineList);
	const mainRawLines = buildLiuyaoLineRecords(originalHexagram, originalHexagram, eightChar, movingLines, true);
	const hidden = calculateLiuyaoHiddenGods(mainRawLines, originalHexagram, eightChar);
	const mainLines = withLiuyaoHiddenGods(mainRawLines, hidden);
	const changedLines = buildLiuyaoLineRecords(changedHexagram, originalHexagram, eightChar, new Set(), false);
	const bodies = formatLiuyaoBodies(originalHexagram, mainLines);
	return {
		question,
		datetime,
		eightChar,
...
function formatLiuyaoChartMarkdown(data: ReturnType<typeof buildLiuyaoChartData>) {
	const pillars = getPillarCycles(data.eightChar);
	return joinSections([
		'六爻排盘',
		[
			`所问之事：${data.question}`,
			`时间：${data.datetime}`,
			`干支:${pillars.year.getName()}年 ${pillars.month.getName()}月 ${pillars.day.getName()}日 ${pillars.hour.getName()}时`,
			`空亡：年空亡${liuyaoKongwangText(pillars.year)} 月空亡${liuyaoKongwangText(pillars.month)} 日空亡${liuyaoKongwangText(pillars.day)} 时空亡${liuyaoKongwangText(pillars.hour)}`,
...
function registerLiuyaoTools(server: McpServer) {
	server.registerTool(
		'liuyao_chart',
		{
			title: '六爻排盘',
			description: '适用场景: 已有占卜问题、占卜时间、本卦和变卦名称时，生成六爻装卦底稿。不要直接输出最终吉凶断语。下一步: 根据所问之事取用神，再结合世应、动变、日月、空亡、伏神和长生做人工解读。',
			inputSchema: {
				question: z.string().describe('占卜所问的具体问题，建议一事一问'),
				datetime: z.string().describe('占卜日期时间 YYYY-MM-DD HH:MM，按起卦当地民用时间输入'),
				originalHexagram: z.string().describe('本卦六十四卦名称，如 乾为天、火地晋、火泽睽'),
				changedHexagram: z.string().describe('变卦六十四卦名称，如 坤为地、火雷噬嗑；静卦可与本卦相同'),
			},
		},
		async ({ question, datetime, originalHexagram, changedHexagram }) => {
			try {
				const data = buildLiuyaoChartData(question, datetime, originalHexagram, changedHexagram);
				return textResult(formatLiuyaoChartMarkdown(data));
			} catch (e) {
				return errResult(`liuyao_chart 错误: ${e instanceof Error ? e.message : String(e)}`);
			}
		},
	);
}
